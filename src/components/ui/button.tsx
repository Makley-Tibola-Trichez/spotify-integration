...
const buttonVariants = cva(
	"inline-flex items-center justify-center whitespace-nowrap rounded-3xl text-md font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50",
	{
		variants: {
			variant: {
	...
	({ className, variant, size, asChild = false, ...properties }, reference) => {
		...
		return (
			<Comp className={cn(buttonVariants({ variant, size, className }))} ref={reference} {...properties}>
				<span className="px-8 font-bold">{properties.children}</span>
			</Comp>
		);
	},
);
